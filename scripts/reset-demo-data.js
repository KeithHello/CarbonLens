...
function buildEntry(date, index, scenario, sessionPrefix) {
  ...
  return {
    ...
    comparison: {
      global_percentile: Math.min(98, Math.max(5, Math.round((total / 18.3) * 55))),
      national_percentile: Math.min(98, Math.max(5, Math.round((total / 22.3) * 50))),
      vs_personal_avg: round(total / previousThirtyDayAverage, 2),
      global_avg_kg: 18.3,
      national_avg_kg: 22.3,
    },
